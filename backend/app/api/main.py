...
try:
    ...
    from evolution import EvolutionEngine, EvolutionConfig, Individual
    from evaluation import evaluate_individual, TestResult
    EVOFORGE_AVAILABLE = True
    logger.info("成功导入真实EvoForge引擎 - 无模拟数据")
except ImportError as e:
    logger.error(f"无法导入EvoForge模块: {e}")
    logger.error("请确保所有依赖已正确安装")
    EVOFORGE_AVAILABLE = False
    
    # 如果无法导入真实引擎，则退出程序
    logger.critical("无法启动EvoForge API服务 - 缺少必要的引擎模块")
    logger.critical("请确保engine.py、evolution.py和evaluation.py文件存在且可导入")
    raise ImportError("EvoForge引擎模块不可用")
...
@api_router.post("/simulation/control")
...
@api_router.get("/simulation/cells")
...
@api_router.get("/simulation/molecules")
...
@api_router.get("/simulation/analytics")
...
@api_router.get("/system/activities")
async def get_system_activities():
    ...
    activity_types = [
        "细胞分裂", "基因突变", "适应度评估", "代数完成", 
        "种群更新", "交叉繁殖", "环境变化", "资源分配"
    ]
    
    for i in range(10):
        activity_time = now - timedelta(minutes=random.randint(1, 60))
        activities.append({
            "id": f"activity_{i}",
            "type": random.choice(activity_types),
            "description": f"系统执行了{random.choice(activity_types)}操作",
            "timestamp": activity_time.isoformat(),
            "status": random.choice(["success", "warning", "info"]),
            "details": {
                "generation": random.randint(1, 100),
                "cell_count": random.randint(50, 200),
                "fitness_score": round(random.uniform(0.1, 1.0), 3)
            }
        })
...
async def websocket_endpoint(websocket: WebSocket):
                ...
                if evoforge_engine and evoforge_engine.is_running:
                    ...
                    await websocket.send_json({
                        ...
                        "data": {
                            ...
                            "currentPopulation": stats.current_population,
                            "bestFitness": stats.best_fitness,
                            "averageFitness": stats.average_fitness,
                            "diversityIndex": stats.diversity_index,
                            "totalMolecules": stats.total_molecules,
                            "activeReactions": stats.active_reactions,
                            "collisionCount": stats.collision_count,
                            "livingCells": stats.living_cells,
                            "generatedPrograms": stats.generated_programs,
                            "successfulCompilations": stats.successful_compilations,
                            "optimizationImprovements": stats.optimization_improvements,
                            "fps": stats.fps,
                    ...
                    if stats.generation > last_generation:
                        await websocket.send_json({
                            ...
                            "data": {
                                ...
                                "best_fitness": stats.best_fitness,
                                "average_fitness": stats.average_fitness,
                                "population_size": stats.current_population,
    ...
    try:
        ...
        # 启动模拟更新任务
        update_task = asyncio.create_task(send_simulation_updates())
            ...
            try:
                ...
                try:
                    ...
                    if message.get("type") == "ping":
                        ...
                        if evoforge_engine:
                            await evoforge_engine.start()
                            await websocket.send_json({"type": "simulation_started"})
                    elif message.get("type") == "pause_simulation":
                        if evoforge_engine:
                            await evoforge_engine.pause()
                            await websocket.send_json({"type": "simulation_paused"})
                    elif message.get("type") == "stop_simulation":
                        if evoforge_engine:
                            await evoforge_engine.stop()
                            await websocket.send_json({"type": "simulation_stopped"})
